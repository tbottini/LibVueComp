...
class MailSender
{
	...
	send(subject, addr, content, attachments, sender = "no-rply@atelier-azurees.com")
	{
		var mailOptions = {
			from: sender,
			to: addr,
	...
	{
		var content = fs.readFileSync('./views/mail/resetPassword.html')
			.toString().split('<token>').join(token);
	...
	{
		var content = fs.readFileSync(__dirname + '/../views/notifMail.html')
			.toString()
